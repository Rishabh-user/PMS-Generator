...
_DS_FILL_RULE = PatternFill("solid", fgColor="C6EFCE")     # green  — Rule Extraction
_DS_FILL_RAG = PatternFill("solid", fgColor="BDD7EE")      # blue   — LLM (RAG)
_DS_FILL_NO_RAG = PatternFill("solid", fgColor="FFE699")   # yellow — LLM (No RAG)
_DS_FILL_UNKNOWN = PatternFill("solid", fgColor="F2F2F2")  # grey   — unknown / cached

_DS_SOURCE_FILL = {
    "Rule Extraction": _DS_FILL_RULE,
    "LLM (RAG)": _DS_FILL_RAG,
    "LLM (No RAG)": _DS_FILL_NO_RAG,
}

# Per-source fallback description when no section-specific override exists.
_DS_SOURCE_DESC = {
    "Rule Extraction": "Deterministic — from project catalogue or ASME dimensional standard tables",
    "LLM (RAG)": "AI-generated by Claude with retrieved ASME standard context (RAG enabled)",
    "LLM (No RAG)": "AI-generated by Claude without external knowledge retrieval (RAG disabled)",
}

# Section-specific description overrides: {section: {source: description}}
_DS_SECTION_DESC: dict[str, dict[str, str]] = {
    "Pressure-Temperature Rating": {
        "Rule Extraction": "Loaded from pipe_classes.json catalogue (ASME B16.5 / API 6A rated values)",
        "LLM (RAG)":       "ASME B16.5 / API 6A / B31.3 tables — AI-generated with retrieved standard context (custom class)",
        "LLM (No RAG)":    "ASME B16.5 / API 6A / B31.3 — AI-generated from training knowledge (custom class)",
    },
    "Hydrotest Pressure": {
        "Rule Extraction": "ASME B31.3 §345.4.2(b): 1.5 × P × (S_T / S_design) — computed server-side",
        "LLM (RAG)":       "AI-generated: 1.5 × AI P-T max pressure (custom class, RAG-assisted)",
        "LLM (No RAG)":    "AI-generated: 1.5 × AI P-T max pressure (custom class, training knowledge)",
    },
    "Pipe Data — Dimensions (OD / WT)": {
        "Rule Extraction": "OD and wall thickness corrected server-side from ASME B36.10M / B36.19M lookup tables",
        "LLM (RAG)":       "OD and wall thickness from ASME B36.10M / B36.19M — AI-selected schedule, server-corrected",
        "LLM (No RAG)":    "OD and wall thickness from ASME B36.10M / B36.19M — AI-selected schedule, server-corrected",
    },
    "Pipe Data — Type, MOC, Schedule, Ends": {
        "Rule Extraction": "Pipe type, material spec and schedule from project catalogue",
        "LLM (RAG)":       "Pipe type, MOC, schedule and end prep — AI-generated with retrieved ASME B36.10M / B36.19M context",
        "LLM (No RAG)":    "Pipe type, MOC, schedule and end prep — AI-generated from Claude training knowledge",
    },
    "Fittings": {
        "Rule Extraction": "Fitting specs from project catalogue",
        "LLM (RAG)":       "Fitting type, MOC and standards (B16.9 / B16.11 / API 6A) — AI-generated with retrieved ASME context",
        "LLM (No RAG)":    "Fitting type, MOC and standards (B16.9 / B16.11 / API 6A) — AI-generated from Claude training knowledge",
    },
    "Flange": {
        "Rule Extraction": "Flange specs from project catalogue",
        "LLM (RAG)":       "Flange face type, MOC and standard (ASME B16.5 / API 6A) — AI-generated with retrieved context",
        "LLM (No RAG)":    "Flange face type, MOC and standard (ASME B16.5 / API 6A) — AI-generated from Claude training knowledge",
    },
    "Spectacle Blind": {
        "Rule Extraction": "Spectacle blind specs from project catalogue",
        "LLM (RAG)":       "Spectacle blind MOC and standard (ASME B16.48) — AI-generated with retrieved ASME context",
        "LLM (No RAG)":    "Spectacle blind MOC and standard (ASME B16.48) — AI-generated from Claude training knowledge",
    },
    "Bolts / Nuts / Gaskets": {
        "Rule Extraction": "Bolt/nut/gasket specs from project catalogue",
        "LLM (RAG)":       "Stud bolts, hex nuts and gasket type (B16.20 / B16.21 / API 6A) — AI-generated with retrieved context",
        "LLM (No RAG)":    "Stud bolts, hex nuts and gasket type (B16.20 / B16.21 / API 6A) — AI-generated from Claude training knowledge",
    },
    "Valves": {
        "Rule Extraction": "Valve specifications from project catalogue",
        "LLM (RAG)":       "Valve type codes and specs (API 6D / API 600 / API 6A) — AI-generated with retrieved API standard context",
        "LLM (No RAG)":    "Valve type codes and specs (API 6D / API 600 / API 6A) — AI-generated from Claude training knowledge",
    },
    "Notes": {
        "Rule Extraction": "Engineering notes from project catalogue",
        "LLM (RAG)":       "Service-specific notes (NACE MR0175 / B31.3 / material limits) — AI-generated with retrieved context",
        "LLM (No RAG)":    "Service-specific notes (NACE MR0175 / B31.3 / material limits) — AI-generated from Claude training knowledge",
    },
}


def _write_data_sources_section(ws, pms: PMSResponse, row: int, total_cols: int) -> int:
    """Append a colour-coded Data Sources audit table to the active worksheet.

    Columns: Section (cols 1-9) | Data Source (cols 10-14) | Description (15-end).
    Returns the next available row after the table.
    """
    sec_end = min(9, total_cols)
    src_start = min(10, total_cols)
    src_end = min(14, total_cols)
    desc_start = min(15, total_cols)
    desc_end = total_cols

    # Section header (full-width, same style as other sections)
    _write_section_header(ws, row, "Data Sources", col_start=1, col_end=total_cols)
    row += 1

    # Column header row
    col_groups = [
        (1, sec_end, "SECTION"),
        (src_start, src_end, "DATA SOURCE"),
        (desc_start, desc_end, "DESCRIPTION"),
    ]
    for cs, ce, label in col_groups:
        cell = ws.cell(row=row, column=cs)
        cell.value = label
    ...
    def _fill_merged(r, cs, ce, value, font, fill, alignment=LEFT):
    ...
    if not pms.data_sources:
        _fill_merged(row, 1, total_cols,
                     "Data source information not available — regenerate this class to populate.",
                     NOTE_FONT, _DS_FILL_UNKNOWN)
        return row + 1

    ai_notes = getattr(pms, "data_source_notes", {}) or {}

    for section, source in pms.data_sources.items():
        fill = _DS_SOURCE_FILL.get(source, _DS_FILL_UNKNOWN)
        # AI self-annotation takes priority; fall back to hardcoded descriptions.
        desc = (
            ai_notes.get(section)
            or _DS_SECTION_DESC.get(section, {}).get(source)
            or _DS_SOURCE_DESC.get(source, "")
        )

        _fill_merged(row, 1, sec_end, section, LABEL_FONT, fill)
        _fill_merged(row, src_start, src_end, source,
                     Font(name="Arial", bold=True, color="1F4E79", size=9), fill, CENTER)
        _fill_merged(row, desc_start, desc_end, desc,
                     Font(name="Arial", italic=True, color="333333", size=9), fill)
        approx_chars_per_line = max(1, (desc_end - desc_start + 1) * 6)
        line_count = 0
        for segment in (desc or "").split("\n"):
            line_count += max(1, (len(segment) + approx_chars_per_line - 1) // approx_chars_per_line)
        ws.row_dimensions[row].height = max(18, line_count * 15)
        row += 1
...
def generate_pms_excel(pms: PMSResponse, output_path: Path) -> Path:
    ...
    # === DATA SOURCES AUDIT TABLE (optional) ===
    if settings.show_data_source_table:
        row = _write_data_sources_section(ws, pms, row, total_cols)
